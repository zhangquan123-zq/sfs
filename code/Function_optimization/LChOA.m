%% 加入凸透镜成像反向学习策略的黑猩猩算法(LChOA)
function [Attacker_score, Attacker_pos, Convergence_curve] = LChOA(X, N, Max_iter, lb, ub, dim, fobj)
...
while l < Max_iter
    ...
    new_Attacker_pos = (lb+ub)/2+(lb+ub)/(2*eta)-Attacker_pos/eta;
    ...
    disp(['LChOA: At iteration ', num2str(l), ' ,the best fitness is ', num2str(Convergence_curve(l))]);
end
